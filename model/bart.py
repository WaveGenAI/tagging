...
from .config import MusicCapsConfig
from .modules import AudioEncoder
from transformers import BartForConditionalGeneration, BartTokenizer, BartConfig, PreTrainedModel


class BartCaptionModel(PreTrainedModel):

    config_class = MusicCapsConfig
    def __init__(self, config,):
        super().__init__(config)
        # non-finetunning case
        bart_config = BartConfig.from_pretrained(config.bart_type)
        self.tokenizer = BartTokenizer.from_pretrained(config.bart_type)
        self.bart = BartForConditionalGeneration(bart_config)

        self.n_sample = config.sr * config.duration
        self.hop_length = int(0.01 * config.sr)  # hard coding hop_size
        self.n_frames = int(self.n_sample // self.hop_length)
        self.num_of_stride_conv = config.num_of_conv - 1
        self.n_ctx = int(self.n_frames // 2 ** self.num_of_stride_conv) + 1
        self.audio_encoder = AudioEncoder(
            n_mels=config.n_mels,  # hard coding n_mel
            n_ctx=self.n_ctx,
        ...
        self.loss_fct = nn.CrossEntropyLoss(label_smoothing=config.label_smoothing, ignore_index=-100)

    def _init_weights(self, module):
        std = 0.02  # self.config.init_std
